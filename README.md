...
├── 📄 list.txt                         # Daftar URL target
├── 📄 komen.txt                        # Database komentar dengan keyword links
├── 📄 akun.txt                         # Database akun untuk login
├── 📄 requirements.txt                 # Dependencies Python
├── 📄 clear.py                         # Browser cleanup utilities
├── 📄 bot.log                          # Log file aplikasi
│
├── 📁 cache/                           # Cache & Learning System
│   ├── 📄 template_cache.json          # Cache deteksi template (otomatis terbuat sendiri)
│   ├── 📄 element_cache.json           # Cache element selector (otomatis terbuat sendiri)
│   ├── 📄 success_learning_data.json   # Data pembelajaran (otomatis terbuat sendiri)
│   └── 📄 domain_success_patterns.json # Pattern domain (otomatis terbuat sendiri)
│
├── 📁 templates/                   # Template Website
│   ├── 📁 Wordpress/               # Template WordPress
│   │   ├── 📄 config.yaml          # Konfigurasi template
│   │   ├── 📄 actions.py           # Bot actions & element caching
│   │   ├── 📄 smart_detection.py   # Smart success detection & learning
│   │   └── 📄 signature.yaml       # Comment signatures
│   │   
│   │
│   ├── 📁 Wix/                     # Template Wix
│   │   ├── 📄 config.yaml          # Konfigurasi template
│   │   ├── 📄 actions.py           # Bot actions & popup handlers
│   │   ├── 📄 popups.py            # Advanced popup handlers
│   │   └── 📄 signature.yaml       # Comment signatures
│   │
│   └── 📁 website3/                # Template Forum
│       ├── 📄 config.yaml          # Konfigurasi template
│       ├── 📄 actions.py           # Bot actions
│       └── 📄 bbcode_handler.py    # BBCode support
│
└── 📁 results/                                 # Hasil Eksekusi
    └── 📁 YYYYMMDD_HHMMSS/                     # Folder hasil berdasarkan timestamp
        ├── 📄 success_YYYYMMDD_HHMMSS.txt      # URL yang berhasil
        ├── 📄 failed_YYYYMMDD_HHMMSS.txt       # URL yang gagal
        ├── 📄 no_template_YYYYMMDD_HHMMSS.txt  # URL tanpa template
        └── 📄 summary_YYYYMMDD_HHMMSS.txt      # Ringkasan eksekusi
```
...
### 1. Clone Repository
```bash
git clone https://github.com/Dzbackdor/Bx-Website-Optimization.git
cd Bx-Website-Optimization
```
...
### 2. **list.txt** - Daftar URL Target (WAJIB)
```
...
### 3. **komen.txt** - Template Komentar dengan Keyword Links (WAJIB)
```
...
```bash
python main.py [OPTIONS] [COMMAND] [FILE]

OPTIONS:
  --headless      Jalankan browser di background (tanpa tampilan)
  --gui           Mode GUI (mengaktifkan tampilan browser)
  --no-resume     Load semua url dari awal (tidak filter yang sudah diproses)

COMMANDS:
  test <url>      Test satu URL saja
  list <file>     Gunakan file URL custom

EXAMPLES:
  python main.py                                    # Mode default dengan resume
  python main.py --headless                         # Background mode
  python main.py test https://example.com           # Test satu URL
  python main.py list url.txt                       # File custom dengan resume
  python main.py --no-resume list url.txt           # File custom tanpa resume
  python main.py --headless list url.txt            # Background + file custom
```
...
akun.txt          # Berisi password
bot.log           # Berisi aktivitas detail
cache/            # Berisi data learning
results/          # Berisi hasil eksekusi
```
...
### ✅ **File Wajib:**
- [ ] `config.yaml` - Data user sudah diisi (name, email, website)
- [ ] `list.txt` - Minimal 1 URL target
- [ ] `komen.txt` - Minimal 1 template komentar dengan format `{text|url}`
- [ ] `akun.txt` - Format `email:password` (opsional)
- [ ] `requirements.txt` - Dependencies terinstall
...
### v1.0.0 (Current)
- ✅ Multi-template support (WordPress, Wix)
- ✅ Smart auto-detection system dengan machine learning
- ✅ Advanced caching & learning system
- ✅ Resume feature untuk melanjutkan proses
- ✅ Command line interface lengkap
- ✅ Keyword links dengan multi-format support
- ✅ Comprehensive logging & reporting
- ✅ Advanced popup handling system
- ✅ Browser cleanup & optimization
- ✅ Chrome version detection & compatibility
- ✅ URL tracking dengan redirect detection
- ✅ Smart link processing per platform
